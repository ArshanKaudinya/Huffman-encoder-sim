...
fn chunker_thread(
    ...
    for chunk in input.chunks(chunk_bytes) {
        ...
        // Send to next stage
        tx.send(frame).map_err(|_| Error::Channel("chunker channel closed".to_string()))?;
...
fn packetizer_thread(
    ...
    while let Ok(chunk_frame) = rx.recv() {
        ...
        for packet in packets {
            tx.send(packet).map_err(|_| Error::Channel("packetizer channel closed".to_string()))?;
        }
...
fn network_thread(
    ...
    // Use Arc+Mutex to share the simulator between threads
    let sim = Arc::new(Mutex::new(NetworkSimulator::new(config)));

    // Main loop: feed packets into simulator first
    while let Ok(packet) = rx.recv() {
        metrics.lock().unwrap().packets_sent += 1;
        sim.lock().unwrap().send(packet);
    }

    // Now drain all packets from the simulator
    loop {
    ...
    metrics.lock().unwrap().packets_dropped = stats.packets_dropped;
        ...
        match rx.recv_timeout(Duration::from_millis(100)) {
            Ok(packet) => {
                ...
                match reassembler.insert_packet(packet) {
                    Ok(Some((chunk_id, frame_bytes))) => {
                        ...
                        tx.send((chunk_id, frame_bytes))
                            .map_err(|_| Error::Channel("receiver channel closed".to_string()))?;
            ...
            Err(crossbeam_channel::RecvTimeoutError::Timeout) => {
                ...
                if !errors.is_empty() {
                    ...
                    drop(tx);
                    return Err(Error::Channel("chunks timed out".to_string()));
                }
